...
def draw_points(points):
    ...
    map_to = {(i, j): '-' for i in range(1, 4) for j in range(1, 4)}

    for point in points:
        point_list = (point['x'], point['y'])
        map_to[point_list] = str(point['class'])

    print('y=3  ' + map_to[(1, 3)] + '   ' + map_to[(2, 3)] + '   ' + map_to[(3, 3)])
    print('y=2  ' + map_to[(1, 2)] + '   ' + map_to[(2, 2)] + '   ' + map_to[(3, 2)])
    print('y=1  ' + map_to[(1, 1)] + '   ' + map_to[(2, 1)] + '   ' + map_to[(3, 1)])
    print('    x=1 x=2 x=3\n')
